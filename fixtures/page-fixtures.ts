...
export const test = base.extend<PageFixtures>({
  ...
  setupPage: async ({ page, captchaPrevention }: { page: Page, captchaPrevention: any }, use: (r: Page) => Promise<void>) => {
  ...
  googleHome: async ({ setupPage }: { setupPage: Page }, use: (r: GoogleHomePage) => Promise<void>) => {
    const googleHome = new GoogleHomePage(setupPage);
    await use(googleHome);
  },

  // Search Results Page fixture
  searchResults: async ({ setupPage }: { setupPage: Page }, use: (r: GoogleSearchResultsPage) => Promise<void>) => {
    const searchResults = new GoogleSearchResultsPage(setupPage);
    await use(searchResults);
  },

  // CAPTCHA Handler fixture
  captchaHandler: async ({ setupPage }: { setupPage: Page }, use: (r: CaptchaHandler) => Promise<void>) => {
